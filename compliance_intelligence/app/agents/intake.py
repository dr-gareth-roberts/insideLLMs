...
def run_intake(state: PipelineState) -> PipelineState:
    ...
    state.processing_steps.append(f"[{_now()}] Intake: received transaction {state.transaction.transaction_id}")
    ...
    if txn.amount_usd_approx >= HIGH_VALUE_THRESHOLD_USD:
        ...
        state.processing_steps.append(f"[{_now()}] Intake: flagged high-value (${txn.amount_usd_approx:,.2f})")
    ...
    for label, entity in [("source", txn.source_entity), ("destination", txn.destination_entity)]:
        if entity.inherent_risk in (RiskLevel.HIGH, RiskLevel.CRITICAL):
            alerts.append(Alert(
                severity=AlertSeverity.CRITICAL if entity.inherent_risk == RiskLevel.CRITICAL else AlertSeverity.WARNING,
