...
- **Residual failures with `[all]`**: even with `[all]`, ~6 tests fail and are NOT regressions: 5 `test_visualization_coverage.py::TestExperimentExplorer` cases need the pandas Styler (`jinja2>=3.1.5`, not a declared dependency), and `test_tuf_client.py::test_fetch_dataset_refuses_implicit_mock` is a signing/mock-verification expectation. Expect `6864 passed, 6 failed, 92 skipped` on a clean `[all]` install.
