...
- **Residual failures with `[all]`**: even with `[all]`, ~5 tests fail and are NOT regressions: the `test_visualization_coverage.py::TestExperimentExplorer` cases need the pandas Styler (`jinja2>=3.1.5`, not a declared dependency). (`test_tuf_client.py` no longer depends on `tuf` availability: `fetch_dataset` refuses without `allow_mock=True` regardless of whether `tuf` is installed.)
