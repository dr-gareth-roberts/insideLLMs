...
Notes
-----
Pydantic is a required dependency. Configuration always receives validation.
Legacy builder classes remain validated and expose explicit runtime conversion.
...
from insideLLMs.config_schema import RuntimeConfiguration

# Retained for imports from older clients; validation is now always installed.
PYDANTIC_AVAILABLE = True


def _require_pydantic() -> None:
    """Compatibility helper; Pydantic is a required package dependency."""
...
class ModelConfig(BaseModel):
    ...
    model_config = ConfigDict(
        hide_input_in_errors=True, extra="allow", validate_default=True, protected_namespaces=()
    )
...
class DatasetConfig(BaseModel):
    ...
    path: Optional[str] = Field(default=None, description="Path to dataset file (for file source)")
...
class ExperimentConfig(BaseModel):
    ...
    def to_runtime_config(self) -> dict[str, Any]:
        """Convert a legacy builder to the validated version 1 runtime shape.

        Unsupported legacy settings raise rather than being silently ignored.
        Credentials stay as environment variable names until model creation.
        """
        from insideLLMs.config_schema import normalize_runtime_config

        return normalize_runtime_config(self.model_dump(mode="json"))


def load_config_from_yaml(path: Union[str, Path]) -> Union[ExperimentConfig, RuntimeConfiguration]:
...
def load_config_from_json(path: Union[str, Path]) -> Union[ExperimentConfig, RuntimeConfiguration]:
...
def load_config(path: Union[str, Path]) -> Union[ExperimentConfig, RuntimeConfiguration]:
...
def _parse_config_dict(data: dict[str, Any]) -> Union[ExperimentConfig, RuntimeConfiguration]:
    ...
    """
    # Modern files share the runtime schema. Old builder-shaped files retain
    # their historical object interface and expose explicit conversion above.
    if "models" in data or "type" in data.get("model", {}):
        from insideLLMs.config_schema import RuntimeConfiguration, normalize_runtime_config

        return RuntimeConfiguration.model_validate(normalize_runtime_config(data))
...
def save_config_to_yaml(
    config: Union[ExperimentConfig, RuntimeConfiguration], path: Union[str, Path]
) -> None:
...
def validate_config(
    config: Union[ExperimentConfig, RuntimeConfiguration, dict[str, Any]],
) -> Union[ExperimentConfig, RuntimeConfiguration]:
    ...
    if isinstance(config, (ExperimentConfig, RuntimeConfiguration)):
        return config
