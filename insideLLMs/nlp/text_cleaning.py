...
# Precompiled at import time so normalize_contractions() avoids rebuilding the
# contraction map and recompiling the (~118-alternation) regex on every call.
_CONTRACTIONS = {
    "ain't": "am not",
    "aren't": "are not",
    "can't": "cannot",
...
_CONTRACTIONS_PATTERN = re.compile(
    r"\b(" + "|".join(_CONTRACTIONS.keys()) + r")\b", re.IGNORECASE
)


def _replace_contraction(match: re.Match) -> str:
    word = match.group(0)
    return _CONTRACTIONS.get(word.lower(), word)
...
def normalize_contractions(text: str) -> str:
    ...
    """
    return _CONTRACTIONS_PATTERN.sub(_replace_contraction, text)
